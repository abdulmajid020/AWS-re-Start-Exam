...
export const ExamSimulationCard: React.FC = () => {
  ...
  const presets = [
    { label: '65 Questions (90m)', count: 65, time: 90 },
    { label: '25 Questions (30m)', count: 25, time: 30 },
    { label: 'All 103 (Untimed)', count: 103, time: 0 },
  ];
    ...
    <div className="rounded-2xl bg-white border border-slate-200/90 p-6 sm:p-8 shadow-sm">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-3 max-w-xl">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1.5 leading-relaxed">
              Standard 90-minute timed assessment with 65 questions sampled across all AWS domains. Scored on the official scaled 100–1000 range.
            </p>
          </div>
        ...
        <div className="flex flex-col gap-2.5 min-w-[260px]">
          <button
            onClick={() => startExamSimulation(65, 90)}
          ...
          >
            ...
            {showCustom ? 'Hide Custom Options' : 'Custom Question Count & Time'}
          </button>
          ...
          {!showCustom && (
            <div className="flex items-center justify-between gap-1.5 pt-1">
              {presets.map((p, idx) => (
                <button
                  key={idx}
                  onClick={() => startExamSimulation(p.count, p.time)}
                  className="flex-1 py-1 px-1.5 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-200 text-[11px] font-medium text-slate-700 hover:text-slate-900 transition-colors text-center"
                >
                  {p.label}
                </button>
              ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              >
                ...
                <option value={103}>103 Questions (All)</option>
              </select>
            </div>
            ...
            <button
              onClick={() =>
                startCustomQuiz(
                  ...
                  `Assessment (${customCount} Questions)`
                )
              }
